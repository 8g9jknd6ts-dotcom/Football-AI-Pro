...
| Rule075 | 生效 | 让球盘面研究必须先读取实际竞彩让球数，再按该让球线的净胜球边界映射；仅在主让一球（-1）时，支持主队恰好赢一球才可写盘面倾向让平，其他让球线不得套用该标签。只有映射后明确支持某一结果时才标注方向，无法区分时写不设盘面方向；研究标签不得替代正式推荐，独立净胜球分布不可审计时仍必须 HANDICAP_DECISION=SKIP。 |

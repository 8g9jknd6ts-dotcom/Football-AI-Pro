...
- Corrected handicap research wording and scope: the “一球小胜对应让平倾向” statement applies only when the actual JCZQ line is home -1. Every other line must be mapped independently from its own goal-margin boundary; genuinely indistinguishable cases use “不设盘面方向”. This research label remains separate from the formal `HANDICAP_DECISION=SKIP` gate.

- Clarified the current operating stance: market odds remain the sole live directional input; historical samples are audit/validation context only and must not reverse or re-weight current picks until a sufficiently recent, same-line validation set is established. The earlier Rule072 calibration wording is superseded by this clarification.
- Distinguished an auditable dated-history distribution from a formally validated handicap recommendation. The distribution may support an offline gate/audit, but it must not alter the current market direction; official handicap recommendations remain gated on league, line and time-ordered historical odds backtesting.
